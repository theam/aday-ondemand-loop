...
class Dataverse::DisplayRepoControllerResolverTest < ActiveSupport::TestCase
  ...
  test 'should return landing path for unknown URL patterns' do
    ...
    expected_url = portal_repo_path(connector_type: ConnectorType::DATAVERSE.to_s, action: 'landing')
  ...
  test 'should handle dataset without persistent ID' do
    ...
    assert result.success?
    assert_equal portal_repo_path(connector_type: ConnectorType::DATAVERSE.to_s, action: 'landing'), result.redirect_url
  end
  ...
  test 'should be able to use portal_repo_path helper' do
    landing_path = portal_repo_path(connector_type: ConnectorType::DATAVERSE.to_s, action: 'landing')
