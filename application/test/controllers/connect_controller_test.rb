...
class ConnectControllerTest < ActionDispatch::IntegrationTest
  def stub_action(result: nil, exception: nil)
    ...
    processor = mock('ConnectProcessor')
    processor.expects(:action).with('landing').returns(action)
    ConnectorClassDispatcher.expects(:connect_connector_processor).with(ConnectorType::DATAVERSE).returns(processor)
  end
  ...
  test 'handle renders template when action succeeds' do
    ...
    get connect_repo_url(connector_type: 'dataverse', action: 'landing')
  ...
  test 'handle redirects with error message when action raises' do
    ...
    get connect_repo_url(connector_type: 'dataverse', action: 'landing')
    assert_redirected_to root_path
    assert_equal I18n.t('connect.handle.message_processor_error', connector_type: 'dataverse', action: 'landing'), flash[:alert]
  end

  test 'redirects when connector type is invalid' do
    get connect_repo_url(connector_type: 'bogus', action: 'landing')
    assert_redirected_to root_path
    assert_equal I18n.t('connect.message_invalid_connector_type', connector_type: 'bogus'), flash[:alert]
  end
end
