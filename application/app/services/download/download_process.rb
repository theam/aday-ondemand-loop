...
module Download
  class DownloadProcess
    ...
    def launch
      ...
      File.open(lock_file, 'w') do |service_lock|
        unless service_lock.flock(File::LOCK_EX | File::LOCK_NB) # Exclusive, non-blocking lock
          log_info('Exit. Other DownloadProcess already running', {pid: process_id, elapsed_time: elapsed_time})
          return
        end
        ...
        log_error('Exit. Error while executing DownloadProcess', {pid: process_id, elapsed_time: elapsed_time}, e)
      ensure
        shutdown
      end
    end
    ...
    def lock_file
      File.join(Configuration.metadata_root, 'download.process.lock')
    end

    private

    def startup
      @services << Download::Command::DownloadCommandServer.new(socket_path: Configuration.download_server_socket_file)
