class DataverseDatasetsController < ApplicationController
  ...
  def init_service
    @service = DataverseDatasetService.new(@dataverse_url)
  end

  def init_project_service
    @project_service = DataverseProjectService.new(@dataverse_url)
  end
  ...
  def find_dataset_by_persistent_id
    ...
    begin
    ...
    rescue DataverseDatasetService::UnauthorizedException => e
      ...
      redirect_back fallback_location: root_path, alert: t(".dataverse_service_error", dataverse_url: @dataverse_url, persistent_id: @persistent_id)
      return
    end
  end
  ...
  def search_files_page
    ...
    begin
      ...
      unless @files_page
        ...
        flash[:alert] = t(".dataset_files_not_found", dataverse_url: @dataverse_url, persistent_id: @persistent_id, page: @page)
      ...
      flash[:alert] = t(".dataverse_service_error_searching_files", dataverse_url: @dataverse_url, persistent_id: @persistent_id, page: @page)
      redirect_to root_path
      return
    end
  end
