module Actions
  class DataverseUploadBatchCreate
    include LoggingCommon
    include DateTimeCommon

    def create(project, request_params)
      remote_repo_url = request_params[:object_url]
      url_data = DataverseUrl.parse(remote_repo_url)

      if url_data.collection?
        collection_service = DataverseCollectionService.new(url_data.dataverse_url)
        ...
        dataset_service = DataverseDatasetService.new(url_data.dataverse_url)
