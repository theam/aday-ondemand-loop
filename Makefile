all:: loop_up
.PHONY: loop_up loop_down atest_up atest_down loop_build remote_dev_build release_build loop_docker_builder clean logs bash test test_bash version release_notes coverage guide guide_dev
...
atest_up: atest_down
	$(ENV) $(COMPOSE_CMD) -f docker-compose.yml -f docker-compose.atest.yml -p loop_passenger up --build || :

atest_down:
	$(ENV) $(COMPOSE_CMD) -f docker-compose.yml -f docker-compose.atest.yml -p loop_passenger down -v || :
